...
- [Critical: Orchestrator API Missing Authentication](#orchestrator-api-missing-authentication-2025-11-26)
...
## Orchestrator API Missing Authentication (2025-11-26)

### Problem

During CLI testing, discovered that the orchestrator API (`src/orchestrator_server.py`) had **zero authentication**. All API endpoints were publicly accessible to anyone with network access:
- `/api/metrics` - View execution metrics
- `/api/execute` - Trigger orchestration runs
- `/api/schedules` - Create/modify schedules
- `/api/analytics` - View analytics data
- `/api/cost` - Query cost data

### Root Cause

The FastAPI server was built without any authentication middleware. The endpoints assumed they would be protected by network-level security, but this wasn't sufficient.

### Solution

1. Created `src/azure_haymaker/orchestrator/auth.py`:
   - Azure AD JWT token validation
   - Validates issuer, audience, expiry, client ID
   - `require_auth` FastAPI dependency

2. Updated `src/orchestrator_server.py`:
   - All `/api/*` endpoints now require `AuthDep = Annotated[dict, Depends(require_auth)]`
   - Health check `/` remains open for Azure health probes

3. Updated CLI (`cli/src/haymaker_cli/auth.py`):
   - Added `ServicePrincipalAuthProvider` for service principal auth
   - Auto-detects credentials from environment variables

### Key Learnings

1. **Always implement auth from day one** - Even internal APIs need authentication
2. **Security reviews should include auth verification** - This was missed in multiple reviews
3. **Health probes need exclusion** - Azure needs unauthenticated health check endpoints
4. **Use `Annotated` pattern for FastAPI** - Avoids Ruff B008 "function call in default" errors

### Next Steps

Configure Azure AD app to expose API (`api://client-id`) for full token validation.
