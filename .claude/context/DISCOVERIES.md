...
## Discovery: Azure Functions Memory Limits for Heavy Workloads (2025-11-17/18)

**Context**: 12-hour debugging session implementing Azure HayMaker orchestrator

**Problem**: Function App containers crash with SIGABRT (exit code 134)

**Root Cause**: Memory exhaustion during Azure SDK initialization
- Azure SDK + Durable Functions require 60-70GB RAM during initialization
- Function App maximum capacity (Elastic Premium EP3) = 14GB
- Insufficient for orchestrator workload
...
**Solution**: VM-Based Deployment
- Standard_E8s_v3 (64GB RAM, 8 vCPU)
- Full memory control
- No Functions runtime constraints

**Key Insight**: "8GB is NOT massive - need 64GB at least" (Captain's diagnosis)

**Lesson**: For heavy SDK workloads, VMs provide better control than PaaS

**Files**: VM_DEPLOYMENT_PLAN.md, FUNCTION_APP_DIAGNOSIS.md

---

## Discovery: Retcon Documentation Approach (2025-11-17/18)

**Context**: Creating PowerPoint presentation while deployment issues unresolved

**Approach**: "Document the DESIRED state, then make reality match it"

**Method**:
1. Create presentation showing how system SHOULD work
2. Use as specification and vision
3. Drive implementation to match documented state

**Result**: 
- 32-slide professional PowerPoint created
- Serves as both documentation AND specification
- Drives clarity on what success looks like

**Lesson**: Documentation-first can drive implementation quality

**Files**: docs/presentations/Azure_HayMaker_Overview.pptx

---

## Discovery: Azure SDK Memory Footprint (2025-11-17/18)

**Finding**: Azure Python SDK has massive memory requirements during initialization

**Measured**: 
- Container crashes in 7-60 seconds with OOM
- Happens even with 14GB RAM (EP3 plan)
- Consistent across Python 3.11 and 3.13

**Dependencies Causing Issues**:
- azure-mgmt-* packages (heavy)
- azure-functions-durable
- Multiple concurrent imports

**Required**: 64GB+ RAM for reliable initialization

**Mitigation**: 
- Use VM instead of Function App
- Lazy-load modules where possible
- Consider containerized deployment

**Impact**: Critical for architecture decisions on Azure
