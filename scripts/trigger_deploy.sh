...
trigger_workflow() {
    ...
    echo "ACR: hm${NAMING_PREFIX}${ENVIRONMENT}<hash> (unique per subscription)"
    echo "Orchestrator: haymaker-fastapi-orch"
    echo ""
    echo "After deployment completes, test with:"
    echo "  # Get the FQDN"
    echo "  az containerapp show --name haymaker-fastapi-orch --resource-group haymaker-${NAMING_PREFIX}-${ENVIRONMENT}-rg --query properties.configuration.ingress.fqdn -o tsv"
    echo ""
    echo "  # Test health"
    echo "  curl https://<fqdn>/"
    echo ""
    echo "  # Use Haymaker CLI"
    echo "  uv run haymaker kw list"
    echo ""
